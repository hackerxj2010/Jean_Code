...
// Named so the declaration does not have to spell out immer's internal types.
export const useChatStore: UseBoundStore<Mutate<StoreApi<ChatStore>, [['zustand/immer', never]]>> = create<ChatStore>()(
  immer((set) => ({
    ...initialState,
    ...
    toggleAgentMode: () =>
      set((state) => {
        ...
        state.agentMode = AGENT_MODES[nextIndex] ?? 'DEFAULT'
