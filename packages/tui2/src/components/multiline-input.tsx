...
>(function MultilineInput(
  ...
  const getOrSetStickyColumn = useCallback(
    (lineStarts: number[], cursorIsChar: boolean): number => {
      ...
      const column =
        lineIndex === -1
          ? 0
          : cursorPosition - (lineStarts[lineIndex] ?? 0) + (cursorIsChar ? -1 : 0)
  ...
  const handleMouseDown = useCallback(
    (event: MouseEvent) => {
      ...
      const lineStartChar = lineStarts[lineIndex] ?? 0
  ...
  const handleNavigationKeys = useCallback(
    (key: KeyEvent): boolean => {
      ...
      const visualLineStart = lineStarts[visualLineIndex] ?? logicalLineStart
      const nextVisualLineStart = lineStarts[visualLineIndex + 1]
      const visualLineEnd = nextVisualLineStart !== undefined ? nextVisualLineStart - 1 : logicalLineEnd
