      ...
      <ErrorBoundary fallback={errorFallback} componentName="AgentChildrenGrid">
        <box style={{ flexDirection: 'column', gap: 0, width: '100%' }}>
          {subGroups.map((group, index) => (
            <GridLayout
              key={group[0] ? getItemKey(group[0]) : index}
              items={group}
...
export const MessageWithAgents = memo(
  ({ message, depth, isLastMessage, availableWidth }: MessageWithAgentsProps): ReactNode => {
    ...
    const { onToggleCollapsed, onBuildFast, onBuildMax, onBuildLite } =
      useMessageBlockStore(
        useShallow((state) => ({
    ...
    const dividerBlock = message.blocks?.length === 1 ? message.blocks[0] : undefined
    if (dividerBlock?.type === 'mode-divider') {
      return (
        <ModeDivider
          key={message.id}
