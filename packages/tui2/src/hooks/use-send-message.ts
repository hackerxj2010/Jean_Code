...
import { getAgentIdForMode, getModeSettings } from '../utils/agent-selection'
...
export const useSendMessage = ({
  ...
  const sendMessage = useCallback<SendMessageFn>(
    async ({ content, agentMode, postUserMessage, attachments }) => {
      ...
      if (!client) {
        logger.error({}, '[send-message] The agent could not be created.')
        // Show error to user instead of silently failing
        setMessages((prev) => [
          ...prev,
          createErrorChatMessage(
            '⚠️ The agent could not start. Run `jean doctor` to see what is missing (a provider key, most often).',
          ),
        ])
        await yieldToEventLoop()
        setTimeout(() => scrollToLatest(), 0)
      ...
      try {
        ...
        const runConfig = createRunConfig({
          ...
          modeSettings: getModeSettings(agentMode),
        })
