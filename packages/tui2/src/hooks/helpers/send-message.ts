...
export const handleRunCompletion = (params: {
  ...
  // A failed turn does not come back here as an error output: the client
  // writes the provider's message into the turn itself, and a thrown error
  // goes to `handleRunError`.
  finalizeQueueState({
    setStreamStatus,
    setCanProcessQueue,
    updateChainInProgress,
