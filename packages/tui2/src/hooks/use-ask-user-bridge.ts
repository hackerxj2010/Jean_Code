...
import type { AskQuestion } from '@codebuff/common/utils/ask-user-bridge'
 ...
 */
function filterRedundantOptions(questions: AskQuestion[]): AskUserQuestion[] {
  return questions.map((question) => ({
    question: question.question,
    ...(question.multiSelect === undefined ? {} : { multiSelect: question.multiSelect }),
    // The `ask` tool allows plain strings and objects side by side, and no
    // options at all (a free-text question); the screen reads one shape.
    options: (question.options ?? [])
      .filter((option) => !isRedundantOption(option))
