...
export function getPathCompletion(inputPath: string): string | null {
  ...
  try {
    ...
    if (matches.length === 1 && onlyMatch !== undefined) {
      let completed = path.join(parentDir, onlyMatch) + path.sep
    ...
    const first = firstMatch.toLowerCase()
    const last = (sortedMatches.at(-1) ?? '').toLowerCase()
    ...
    if (commonLength > partial.length) {
      ...
      const commonPrefix = firstMatch.slice(0, commonLength)
