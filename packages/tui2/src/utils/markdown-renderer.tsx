...
const splitNodesByNewline = (nodes: ReactNode[]): ReactNode[][] => {
  // `current` is always the last line: every new one is pushed and becomes it.
...
const applyInlineFallbackFormatting = (node: MarkdownNode): void => {
  ...
  mutable.children.forEach((child) => {
    if (child.type === 'text') {
      ...
      const only = replacements.length === 1 ? replacements[0] : undefined
      const hasChanges = only?.type !== 'text' || only.value !== child.value
...
const renderTable = (table: Table, state: RenderState): ReactNode[] => {
  ...
  for (const row of rows) {
    for (const [i, cell] of row.entries()) {
      naturalWidths[i] = Math.max(naturalWidths[i] ?? 3, stringWidth(cell || ''))
    }
  }
  ...
  if (totalNaturalWidth <= availableWidth) {
    ...
    for (let i = 0; i < columnWidths.length && remaining > 0; i++) {
      ...
      const natural = naturalWidths[i] ?? 3
      if (width < natural) {
        const add = Math.min(remaining, natural - width)
        columnWidths[i] = width + add
        remaining -= add
      }
    }
  }
  ...
  const wrappedRows: string[][][] = rows.map((row) =>
    Array.from({ length: numCols }, (_, i) => {
      ...
      return wrapText(cellText, columnWidths[i] ?? 3)
    }),
  )
  ...
  wrappedRows.forEach((wrappedCells, rowIdx) => {
    ...
    for (let lineIdx = 0; lineIdx < rowHeight; lineIdx++) {
      for (let cellIdx = 0; cellIdx < numCols; cellIdx++) {
        const colWidth = columnWidths[cellIdx] ?? 3
